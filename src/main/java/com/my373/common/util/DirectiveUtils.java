...
public abstract class DirectiveUtils {
	...
	public static Integer getInt(String name, Map<String, TemplateModel> params) {
		TemplateModel model = params.get(name);
		return getInt(name, model);

	}

	public static Integer getInt(String name, TemplateModel model) {
		if (model == null) {
			return null;
		}
		if (model instanceof TemplateScalarModel) {
			String s = "";
			try {
				s = ((TemplateScalarModel) model).getAsString();
	...
	public static Integer getInt(String string, Environment env, Map params) {

		Integer int0 = null, int1 = null, int2 = null;
		try {
			int1 = DirectiveUtils.getInt(string, env.getVariable(string));
			int2 = DirectiveUtils.getInt(string, params);
		} catch (TemplateModelException e) {
		}
